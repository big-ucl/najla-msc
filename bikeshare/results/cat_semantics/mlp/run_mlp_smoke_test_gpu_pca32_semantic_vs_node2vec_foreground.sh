...
LOG=/home/najla/dev/najla-msc/bikeshare/results/semantic_enhancement/mlp/mlp_smoke_test_gpu_pca32_semantic_vs_node2vec.log
PIDFILE=/home/najla/dev/najla-msc/bikeshare/results/semantic_enhancement/mlp/mlp_smoke_test_gpu_pca32_semantic_vs_node2vec.pid

: > "$LOG"
echo "$$" > "$PIDFILE"
exec > >(tee -a "$LOG") 2>&1

echo "Started MLP smoke test pca32 semantic_vs_node2vec at $(date -Is)"
...
echo "Finished MLP smoke test pca32 semantic_vs_node2vec at $(date -Is) with exit code $status"
exit "$status"