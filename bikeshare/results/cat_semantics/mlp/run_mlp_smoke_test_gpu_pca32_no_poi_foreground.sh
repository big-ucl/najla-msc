...
LOG=/home/najla/dev/najla-msc/bikeshare/results/semantic_enhancement/mlp/mlp_smoke_test_gpu_pca32_no_poi.log
PIDFILE=/home/najla/dev/najla-msc/bikeshare/results/semantic_enhancement/mlp/mlp_smoke_test_gpu_pca32_no_poi.pid

: > "$LOG"
echo "$$" > "$PIDFILE"
exec > >(tee -a "$LOG") 2>&1

echo "Started MLP smoke test pca32 no_poi at $(date -Is)"
...
echo "Finished MLP smoke test pca32 no_poi at $(date -Is) with exit code $status"
exit "$status"