...
# Import os so the wiki embedding mode can be selected from the terminal.
import os
...
# =========================================================
# 4. Single best CatBoost test configuration
# =========================================================

def catboost_test():
    """
    Return the best recorded CatBoost configuration for a three-DF test run.

    This uses the best row from:
        results/catboost/multiReg_catboost_12pm_5yrs_semantic_pca_grid.csv

    The selected row was trial 54 with Avg_RMSE_mean = 5.150952580958721.
    """

    # Return a list because the downstream model loop expects configurations.
    return [
        {
            ...
            # Best semantic-PCA CatBoost grid value.
            "learning_rate": 0.1,

            # Keep the same large tree budget with early stopping.
            "iterations": 3000,
...
def run_one_dataframe_variant(
    ...
    # Use the dedicated CatBoost test mode: one best config on the first fold.
    search_name = "catboost_test"
    parameter_combinations = catboost_test()
...
def main():
    ...
    # Run all three dataframe variants for the CatBoost enhancement test.
    selected_df_indices = [0, 1, 2]
