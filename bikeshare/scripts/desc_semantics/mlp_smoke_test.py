...
def prepare_mlp_fold(
    ...
    *,
    ...
    # Transform targets for MSE training.
...
def fit_one_mlp_fold(
    ...
    *,
    ...
    # Use MSE on log1p-transformed targets.
    criterion = {
        "MSE": nn.MSELoss(),
    }[params["loss_name"]]
    ...
    for epoch in range(1, params["max_epochs"] + 1):
        ...
        for batch_start in range(0, n_train, batch_size):
            ...
            # Clear gradients, calculate log1p predictions and MSE, then update weights.
            optimizer.zero_grad()
        ...
        # Calculate validation MSE on the log1p target scale.
        model.eval()
        with torch.no_grad():
            val_loss = criterion(
...
def main():
    ...
    # Run the active one-trial semantic-branch smoke profile.
    ...
    # Store this run under the active embedding-size folder.
    run_results_dir = RESULTS_DIR / wiki_mode_label
    ...
    for df_indx in selected_df_indices:
        ...
        # Run one comparable parameter set on the first saved SPCV fold.
        ...
        # Keep the current smoke-test filename format.
        result_prefix = (
            f"multiReg_mlp_12pm_5yrs_"
            f"{wiki_mode_label}_"
            f"{feature_set_name}_"
            f"{search_name}_{smoke_profile}"
        )
        grid_results_file = run_results_dir / f"{result_prefix}_grid.csv"
        fold_results_file = run_results_dir / f"{result_prefix}_fold.csv"
        feature_importance_file = run_results_dir / f"{result_prefix}_feature_importance.csv"

        # Keep the MLP-specific parameter lookup and epoch-curve files.
        trials_file = run_results_dir / f"{result_prefix}_trials.csv"
        epochs_file = run_results_dir / f"{result_prefix}_epochs.csv"
        predictions_file = run_results_dir / f"{result_prefix}_predictions.csv"
        ...
        # =================================================
        # Validation trial: train and validate one comparable setup
        # =================================================
