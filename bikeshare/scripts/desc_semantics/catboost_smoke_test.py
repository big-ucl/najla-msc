...
WIKI_PCA_COMPONENTS = 16

# Define the CatBoost description-semantic result base directory.
RESULTS_BASE_DIR = (
    PROJECT_ROOT
    / "results"
    / "desc_semantics"
...
# Initialise the result directory with the default PCA setting.
RESULTS_DIR = RESULTS_BASE_DIR / f"pca{WIKI_PCA_COMPONENTS}"
...
def set_results_dir_for_wiki_mode(
    wiki_embedding_mode,
    n_wiki_components,
    wiki_mode_label,
):
    """Set the result folder from the active semantic representation."""

    global RESULTS_DIR
    global GRID_RESULTS_FILE
    global FOLD_RESULTS_FILE

    if wiki_embedding_mode == "pca":
        result_subdir = f"pca{n_wiki_components}"
    else:
        result_subdir = wiki_mode_label

    RESULTS_DIR = RESULTS_BASE_DIR / result_subdir
    RESULTS_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )
...
def get_wiki_embedding_config():
    """Resolve the fixed smoke-test MiniLM representation."""

    return "pca", WIKI_PCA_COMPONENTS, f"pca{WIKI_PCA_COMPONENTS}_0no_text"
...
def load_model_dataframe():
    ...
    desc_extra_cols = [
        ...
        if col in wiki_embedding_df.columns
    ]
...
def build_iteration_history_rows(
    ...
    learn_iterations = list(
        range(
            0,
            len(validation_metric),
            params["metric_period"],
        )
    )

    if len(learn_iterations) < len(learn_metric):
        learn_iterations.append(
            len(validation_metric) - 1
        )
    ...
    for iteration, validation_score in enumerate(validation_metric):
        ...
        iteration_rows.append(
            {
                ...
                "iterations_without_improvement": iteration - best_recorded_iteration,
...
def main():
    ...
    for df_indx in range(3):
        if df_indx == 0:
            df = full_df
            wiki_cols = full_wiki_cols
            feature_set_name = "df0_full_semantic_embeddings_node2vec"
            feature_set_code = "DF0_FULL"
        elif df_indx == 1:
            df = full_df.drop(columns=full_wiki_cols)
            wiki_cols = []
            feature_set_name = "df1_without_semantic_embeddings"
            feature_set_code = "DF1_NO_TEXT_EMB"
        else:
            df = full_df.drop(columns=[*full_wiki_cols, *node2vec_cols])
            wiki_cols = []
            feature_set_name = "df2_without_semantic_embeddings_node2vec"
            feature_set_code = "DF2_NO_TEXT_EMB_NODE2VEC"
