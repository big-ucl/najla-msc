...
EMBEDDING_FILE = (
    ...
    / "processed"
    / "graph"
    / "all_wikidata_minilm_embeddings.parquet"
)

# Set the MLP results folder.
RESULTS_DIR = PROJECT_ROOT / "results" / "semantic_embedings_test" / "mlp"
...
def prepare_mlp_fold(
    ...
    *,
    ...
    val_metadata = val_df[[date_col, id_col, group_col, *target_cols]].copy().reset_index(drop=True)
    test_metadata = test_df[[date_col, id_col, group_col, *target_cols]].copy().reset_index(drop=True)
...
def main():
    ...
    # Smoke test runs the three dataframe variants.
    for df_indx in range(3):

        # Variant 0 keeps MiniLM embeddings and Node2Vec graph features.
        if df_indx == 0:
            df = full_df
        ...
        # Variant 1 removes MiniLM embeddings but keeps Node2Vec features.
        elif df_indx == 1:
        ...
        # Variant 2 removes both MiniLM embeddings and Node2Vec features.
        else:
        ...
        for fold_definition in active_folds:
